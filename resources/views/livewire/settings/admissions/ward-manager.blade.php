<div>
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="javascript:void(0)" wire:click="create">
                            <i class="ti tabler-plus me-1"></i> Add Ward
                        </a></li>
                ...
                <table class="table table-hover border-top text-nowrap">
                    ...
                    <tbody>
                        @forelse($wards as $ward)
                            <tr>
                                <td class="fw-medium text-heading">{{ $ward->name }}</td>
                                <td>{{ $ward->location ?? 'N/A' }}</td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-1">
                                        <button wire:click="edit({{ $ward->id }})" class="btn btn-sm btn-icon btn-label-primary">
                                            <i class="ti tabler-edit"></i>
                                        </button>
                                        <button wire:click="confirmDelete({{ $ward->id }})" class="btn btn-sm btn-icon btn-label-danger" data-bs-toggle="modal" data-bs-target="#deleteWardModal">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center text-muted py-4">No wards found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <!-- Add/Edit Modal -->
    <div wire:ignore.self class="modal fade" id="wardModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ $editingId ? 'Edit' : 'Add' }} Ward</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                   wire:model="name" placeholder="e.g. Male Surgical Ward">
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Location</label>
                            <input type="text" class="form-control @error('location') is-invalid @enderror" 
                                   wire:model="location" placeholder="e.g. Building B, 2nd Floor">
                            @error('location') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Description</label>
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      wire:model="description" rows="3" placeholder="Additional details..."></textarea>
    ...
    <div wire:ignore.self class="modal fade" id="deleteWardModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Delete Ward</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p>Are you sure you want to delete this ward? All associated beds will also be deleted.</p>
                </div>
                <div class="modal-footer">
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('show-ward-modal', () => {
                wardModal.show();
            });
            
            Livewire.on('hide-ward-modal', () => {
                wardModal.hide();
            });
        });
    </script>
