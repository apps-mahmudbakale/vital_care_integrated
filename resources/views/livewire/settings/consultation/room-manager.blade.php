<div>
    <div class="card h-100">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">Consulting Rooms</h5>
                ...
                <table class="table table-hover border-top text-nowrap">
                    ...
                    <tbody>
                        @forelse($rooms as $room)
                            <tr>
                                <td class="fw-medium text-heading">{{ $room->name }}</td>
                                <td>{{ $room->clinic->name }}</td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-1">
                                        <button wire:click="edit({{ $room->id }})" class="btn btn-sm btn-icon btn-label-primary">
                                            <i class="ti tabler-edit"></i>
                                        </button>
                                        <button wire:click="confirmDelete({{ $room->id }})" class="btn btn-sm btn-icon btn-label-danger" data-bs-toggle="modal" data-bs-target="#deleteRoomModal">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center text-muted py-4">No rooms found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <!-- Add/Edit Modal -->
    <div wire:ignore.self class="modal fade" id="roomModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ $editingId ? 'Edit' : 'Add' }} Consulting Room</h5>
                ...
                <form wire:submit.prevent="save">
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('clinic_id') is-invalid @enderror" wire:model="clinic_id">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                   wire:model="name" placeholder="e.g. Room 101">
    ...
    <!-- Delete Modal -->
    <div wire:ignore.self class="modal fade" id="deleteRoomModal" tabindex="-1" aria-hidden="true">
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('show-room-modal', () => roomModal.show());
            Livewire.on('hide-room-modal', () => roomModal.hide());
        });
    </script>
</div>
